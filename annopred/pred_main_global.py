...
def annopred_inf(beta_hats, pr_sigi, h2,n=1000, reference_ld_mats=None, ld_window_size=100):
    ...
    for i, wi in enumerate(range(0, num_betas, ld_window_size)):
        ...
        logging.debug("before pinv, matrix %s" % (str(A.shape)))
        A_inv = linalg.pinv(A)
        logging.debug("after pinv")
        updated_betas[start_i: stop_i] = sp.dot(A_inv / (1.0/n) , beta_hats[start_i: stop_i])  # Adjust the beta_hats

    return updated_betas


def my_mcmc_wrapper(i):
    global jobdicts
    d=jobdicts[i]
    logging.debug('In my_mcmc_wrapper global version %d' % d['ii'])
...
def annopred_genomewide(data_file=None, ld_radius = None, ld_dict=None, out_file_prefix=None, ps=None, 
    ...
    logging.debug('Parallel Global version')
    prf_chr = PRF['chrom']
    prf_sids = PRF['sids']
    prf_pi = PRF['pi']
    prf_sigi2 = PRF['sigi2']
...
#                    logging.debug('The R2 prediction accuracy of PRS using %s was: %0.4f' %(chrom_str, r2_inf))
        ...
        global jobdicts
        jobdicts = [] # this will create a dict for each job
        jobs = [] # this is just the indices
        ...
        for ii, chrom_str in enumerate(chromosomes_list): ## RDB
            if chrom_str in cord_data_g.keys():
                ...
                jobdicts.append(dict(zip(['ii', 'beta_hats', 'Pi', 'Sigi2', 'sig_12', 'start_betas', 'h2', 'n', 'ld_radius', 'num_iter', 'burn_in', 'zero_jump_prob', 'ld_dict'], 
        ...
        ## TESTING
        #for job in jobs:
        #    logging.debug("job %d size %d" % (job, len(cPickle.dumps(jobdicts[job]))))
...
#@profile
def main(p_dict):
    logging.debug("Using pred_main_global")
